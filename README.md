...
一个专注校园内部、重信息撮合、轻图片展示的二手交易平台。通过"纯文本流+结构化标签+同校实名+信用分体系"，实现极低门槛发布与快速、安全的线下撮合。
...
新用户注册获得基础分 100 分。信用分低于 60 分时，将被限制发布权限。
